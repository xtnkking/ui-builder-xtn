...
- `DataTable` owns desktop columns, sorting, loading skeletons, empty/error states, developer-pinned and hidden columns, mobile row rendering, and the optional pager within the same 12px rounded frame. Mark a leading checkbox column with `kind: "selection"`; the first two visible columns then pin to the start by default. `pin: false`, `pin: "start"`, and `pin: "end"` are explicit developer overrides, and a hidden selection column does not trigger automatic pinning. Paginated tables default to a fixed viewport sized from five standard row-height units, independent of `pageSize`, so two-row results, sparse last pages, 20/50-row pages, empty results, and zero-row errors keep the frame and pager stable without creating giant blank surfaces. Keep the typed pagination object present across those request states. `viewportRows` selects another fixed standard-row-height capacity; `viewportRows="auto"` restores content height. Custom cells may be taller and scroll within the fixed surface; content is never clipped to force the exact row count. Unpaginated tables remain automatic unless given a numeric `viewportRows`. Every actually scrollable surface is keyboard-focusable and carries an accessible name. A fixed viewport resets `scrollTop` for new requests, pages, page sizes, or row identity sequences without resetting horizontal scroll. Rows and request state still come from the caller; `loadingRows` controls skeleton count only.
...
- `Pagination` owns page, previous, next, page-size, per-target loading geometry, and its result summary; use `summary` only for a meaningful pending or no-cache message that must preserve pager geometry. `Tabs`, navigation variants, menus, steppers, commands, and anchors keep selection/focus state in their public APIs.
